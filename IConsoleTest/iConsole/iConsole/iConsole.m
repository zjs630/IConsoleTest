...
NSString * AFPercentEscapedStringFromString_zjs(NSString *string) {
...
- (void)actionSheet:(UIActionSheet *)actionSheet didDismissWithButtonIndex:(NSInteger)buttonIndex
{
	...
	else if (buttonIndex != actionSheet.cancelButtonIndex)
	{
        if (buttonIndex == 1)
        {
            NSString *URLSafeName = AFPercentEscapedStringFromString_zjs([[NSBundle mainBundle] objectForInfoDictionaryKey:@"CFBundleName"]);
            NSString *URLSafeLog = AFPercentEscapedStringFromString_zjs([_log componentsJoinedByString:@"\n"]);
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
	if ((self = [super initWithNibName:nibNameOrNil bundle:nibBundleOrNil]))
	{
...
#if ICONSOLE_ADD_EXCEPTION_HANDLER
        //showAlert:YES 可以阻止直接崩溃，点击继续之后，摇一摇手机可以查看崩溃Log。 add by zjs
        ...
        self.infoString = @"Shake device to hide or show console"; //chang by zjs 提示信息
...
{
#if !defined(DEBUG) //只在DEBUG模式下打印log //add by zjs
    return;
#endif
